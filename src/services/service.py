...
class ListService:
    ...
    async def get_list(self, index: str, sort: str = None) -> Optional:
        # entities = await self._get_from_cache(_id)
        # if not entities:
        entities = await self._get_from_elastic(index, sort)
        if not entities:
            return None
        # await self._put_to_cache(entities)

        return entities

    async def _get_from_elastic(self, index: str, sort: str = None) -> Optional:
        if sort:
            try:
